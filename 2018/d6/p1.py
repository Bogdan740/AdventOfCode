...
grid = [[(".",float("inf"), None) for _ in range(min_x, max_x+1)] for _ in range(min_y, max_y+1)]
...
print(max([x for x in areas_dict.items() if x[0] not in is_infinite ], key=lambda x:x[1])[1])
